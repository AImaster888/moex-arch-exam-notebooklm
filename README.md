...
> **專案性質**：開源技術驗證專案（Open Source / Public）  
...
## 🎓 NotebookLM 原生學習工具整合（Studio 實戰技巧）

除了透過 Agent 進行深度分析，NotebookLM 本身內建的 **Studio 學習功能** 也是備考的超級利器：

1. **📝 互動測驗（Quiz / 隨堂測驗）**：
   * 在 NotebookLM 點擊「測驗 (Quiz)」，系統會自動從上傳的歷屆考題與法規資料中提煉出選擇題與簡答題。
   * 適合在讀完特定年份或科目後，進行 5~10 分鐘的即時盲測，快速驗收記憶弱點。
2. **📖 學習指南（Study Guide）**：
   * 一鍵將龐雜的歷屆試題萃取成結構化的「考點複習精華」、「核心術語對照表」與「關鍵概念整理」。
3. **🎙️ 語音概覽（Audio Overview / AI 雙人 Podcast）**：
   * 將歷年考古題與解題邏輯轉化為生動的雙人對話 Podcast。
   * 適合在**通勤、散步或運動時用「聽」的方式複習**，強化對建築法規、工法及歷年出題脈絡的聽覺記憶。
4. **💡 常見問答（FAQ）與時間軸**：
   * 自動生成各年度高頻問題清單，方便考前快速翻閱、掃除盲區。
